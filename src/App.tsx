...
const WidgetCard = ({
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 overflow-hidden flex flex-col p-3">{children}</div>
...
const App = () => {
      ...
      pages: [
        {
          id: 101,
          title: 'Capítulo 1',
          content: '<h2>O Início</h2><p>A aventura começa...</p>',
        },
      ],
      activePageId: 101,
      x: 50,
  ...
  const saveTemplate = () => {
    ...
    try {
      ...
      setModalMessage({
        type: 'error',
        text: 'Erro! Se tiver imagens grandes no layout, o limite de memória do navegador pode ter sido atingido.',
      });
    }
  };
  ...
  const addWidget = (type) => {
    ...
    if (type === 'note') {
      ...
      newWidget.pages = [
        { id: Date.now() + 1, title: 'Nova Página', content: '' },
      ];
  ...
  const rollDice = (widgetId, sides) => {
    ...
    const newHistory = [`D${sides}: ${val}`, ...(widget.history || [])].slice(
      0,
      4
    );
  ...
  const renderNoteWidget = (widget) => {
    const activePage =
      widget.pages.find((p) => p.id === widget.activePageId) || widget.pages[0];

    const addPage = () => {
      const newPage = {
        id: Date.now(),
        title: `Pág ${widget.pages.length + 1}`,
        content: '',
      };
      updateWidget(widget.id, {
        ...
        <div className="flex overflow-x-auto bg-stone-900 rounded-t-lg border-b border-stone-700 p-1 gap-1 custom-scrollbar">
          {widget.pages.map((page) => (
            <div
              ...
              className={`flex items-center gap-1 px-3 py-1 text-sm rounded cursor-pointer group whitespace-nowrap ${
                widget.activePageId === page.id
          ...
          <button
            onClick={addPage}
            className="px-2 py-1 text-stone-400 hover:text-amber-400 flex-shrink-0"
          >
            <FilePlus size={16} />
          </button>
        </div>
  ...
  const renderTableWidget = (widget) => {
    ...
    const addRow = () =>
      updateWidget(widget.id, {
        rows: [...widget.rows, Array(widget.rows[0].length).fill('')],
      });
    const addColumn = () =>
      updateWidget(widget.id, { rows: widget.rows.map((row) => [...row, '']) });

    const removeRow = (rIdx) => {
      if (widget.rows.length <= 1) return;
      updateWidget(widget.id, {
        rows: widget.rows.filter((_, i) => i !== rIdx),
      });
    };
    ...
    const removeColumn = (cIdx) => {
      ...
      updateWidget(widget.id, {
        rows: widget.rows.map((row) => row.filter((_, i) => i !== cIdx)),
      });
    };
    ...
    return (
      <div className="flex flex-col h-full">
        <div className="mb-2 flex gap-2">
          <button
            onClick={addRow}
            className="text-xs bg-stone-700 hover:bg-stone-600 px-2 py-1 rounded flex items-center gap-1"
          >
            <PlusSquare size={14} /> Linha
          </button>
          <button
            onClick={addColumn}
            className="text-xs bg-stone-700 hover:bg-stone-600 px-2 py-1 rounded flex items-center gap-1"
          >
            <PlusSquare size={14} /> Coluna
          </button>
        </div>
            ...
            <thead>
              ...
              <tr>
                {widget.rows[0].map((_, cIdx) => (
                  <th
                    key={`del-col-${cIdx}`}
                    className="p-1 border-b border-stone-700 bg-stone-800"
                  >
                    <button
              ...
              {widget.rows.map((row, rIdx) => (
                <tr key={`row-${rIdx}`}>
                  {row.map((cell, cIdx) => (
                    <td
                      key={`cell-${rIdx}-${cIdx}`}
                      className="border border-stone-700 p-0 relative"
                    >
                      <input
                        type="text"
                        value={cell}
  ...
  return (
    <div className="flex flex-col h-screen bg-stone-900 text-stone-100 font-sans selection:bg-amber-600 selection:text-white overflow-hidden">
      ...
      <header className="flex flex-col md:flex-row justify-between items-center bg-stone-950 p-4 border-b border-stone-700 z-50 flex-shrink-0 shadow-md gap-4">
        ...
        <div className="flex flex-wrap justify-center gap-2 flex-1 md:justify-start md:ml-4">
          <button
            ...
            className="bg-stone-800 hover:bg-amber-600 transition-colors px-3 py-2 rounded flex items-center gap-2 border border-stone-700 text-sm font-medium"
          >
        ...
        <div className="flex">
          ...
          >
            <FolderOpen size={16} className="text-amber-500" /> Salvar /
            Carregar Layout
          </button>
        </div>
      </header>
          ...
          <div className="bg-stone-900 border border-stone-600 rounded-lg shadow-2xl w-full max-w-md flex flex-col overflow-hidden">
            <div className="flex justify-between items-center p-4 border-b border-stone-700 bg-stone-950">
              <h2 className="text-lg font-bold text-amber-500 flex items-center gap-2">
                <Save size={20} /> Modelos de Escudo
              </h2>
              <button
            ...
            <div className="p-5 flex flex-col gap-6">
              ...
              <div className="flex flex-col gap-2">
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    ...
                    className="bg-amber-600 hover:bg-amber-500 text-stone-900 font-bold px-4 py-2 rounded text-sm transition-colors flex items-center gap-1 shadow"
                  >
                    Salvar
                  </button>
                </div>
                {modalMessage.text && (
                  <div
                    className={`text-xs p-2 rounded mt-1 font-medium flex items-center gap-1 ${
              ...
              <div className="flex flex-col gap-2">
                <label className="text-sm font-bold text-stone-300">
                  Carregar Modelos Salvos ({templates.length})
                </label>
                <div className="flex flex-col gap-2 max-h-56 overflow-y-auto custom-scrollbar pr-1">
                  {templates.length === 0 ? (
                    ...
                    templates.map((t) => (
                      <div
                        key={t.id}
                        className="flex items-center justify-between bg-stone-800 border border-stone-700 p-3 rounded group hover:border-amber-600/50 transition-colors"
                      >
                        <div className="flex flex-col overflow-hidden pr-2">
                          ...
                          <span className="text-xs text-stone-500">
                            {t.date} • {t.widgets.length} Ferramentas na tela
                          </span>
                        </div>
      ...
      <main
        className="relative flex-1 w-full bg-stone-900 overflow-hidden"
        id="desktop-area"
      >
                ...
                {widget.imageData ? (
                  ...
                  <label className="cursor-pointer flex flex-col items-center text-stone-400 hover:text-amber-500 transition-colors w-full h-full justify-center">
                    ...
                    <span className="text-sm text-center px-4">
                      Clique para carregar mapa ou imagem
                    </span>
                    <input
                      type="file"
                      accept="image/*"
